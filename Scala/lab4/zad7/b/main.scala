...
class Int2DArrow(x: Int, y: Int, val width: Int) extends Int2DVec(x, y) {
  override def equals(other: Any): Boolean = {
    //print("[Int2DArrow.equals(other: Any) called] ")
    other match {
      case that: Int2DArrow =>
        (that canEqual this) && (this.width == that.width) && super.equals(that)
      case _ => false
    }
  }
  override def hashCode = {
...
object testSuite {
  ...
  def tc4() {
    ...
    checkPredicate(mutVectors.contains(mutV1), "[Before change] mutVectors.contains(mutV1)")
    mutV1.x *= 2
    checkPredicate((!mutVectors.contains(mutV1)), "[After change] (!mutVectors.contains(mutV1))")
  }
  def tc5() {
    ...
    checkPredicate(!(HashSet[Int2DVec](v1) contains a1), "!(HashSet[Int2DVec](v1) contains a1)")
    checkPredicate(!(HashSet[Int2DVec](a1) contains v1), "!(HashSet[Int2DVec](a1) contains v1)")
  }
  def run() {
    reset()
    try { 
